...
public class PlotSummaries extends FactoidEngine<PlotSummary> {

    @Override
    protected String url(final String query, final int options) {
        return String.format("http://www.imdb.com/search/text?realm=title&&field=plot&&q=%s&ref_=fn_pl", query);
    }
    ...
    @Override
    protected void parseRow(final String query, final int options, final Element tr, final List<PlotSummary> results) {
        ...
        if(!episodes.isEmpty()){
            ...
            results.add(new TVEpisodePlotSummary(titleUrl, titleName, (TV) title, tvEpisode, text));
        }else{
            results.add(new PlotSummary(titleUrl, titleName, title, text));
        }
    }
}
